...
ALLOWED_YEARS = set(range(2000, 2011))
MAX_RESULTS_PER_PAGE = 50
GLOBAL_BESTSELLER_PAGES = 12
GENRE_BESTSELLER_PAGES = 5
...
def fetch_item_list(*, query_type: str, page: int, category_id: int | None = None) -> list[dict]:
    start = ((page - 1) * MAX_RESULTS_PER_PAGE) + 1
    params = {
        "ttbkey": _api_key(),
...
def _is_target_book(item: dict) -> bool:
...
def _link_book_authors(db, book: Book, display_authors: list[str]) -> None:
...
def _upsert_book(db, item: dict, *, isbn13_index, isbn10_index, existing_meta) -> tuple[str, Book | None]:
    isbn13 = item.get("isbn13") or None
    isbn10 = item.get("isbn") or None

    existing_exact = None
    if isbn13 and isbn13 in isbn13_index:
        existing_exact = isbn13_index[isbn13]
    elif isbn10 and isbn10 in isbn10_index:
        existing_exact = isbn10_index[isbn10]

    lookup = None
    if existing_exact and existing_exact.total_pages and existing_exact.description and existing_exact.thumbnail:
        lookup = item
    else:
    ...
    duplicate = existing_exact or _find_duplicate_book(
    ...
    if isbn10:
        ...
        if existing_by_isbn10:
            ...
            return "duplicate", existing_by_isbn10
...
def _candidate_key(item: dict) -> tuple[str, str]:
    isbn13 = item.get("isbn13") or ""
    isbn10 = item.get("isbn") or ""
    if isbn13:
        return ("isbn13", isbn13)
    if isbn10:
        return ("isbn10", isbn10)
    title = _normalize_title(item.get("title") or "")
    authors = "|".join(_parse_author_names(item.get("author")))
    return ("title_author", f"{title}::{authors}")
...
def _collect_candidates() -> list[dict]:
    ...
    for page in range(1, GLOBAL_BESTSELLER_PAGES + 1):
        items = fetch_item_list(query_type="Bestseller", page=page)
        for item in items:
            if not _is_target_book(item):
                continue
    ...
    for _, category_id in GENRE_CATEGORY_IDS.items():
        for page in range(1, GENRE_BESTSELLER_PAGES + 1):
            items = fetch_item_list(query_type="Bestseller", page=page, category_id=category_id)
            for item in items:
                if not _is_target_book(item):
                    continue
                key = _candidate_key(item)
...
def import_broad_korean_books() -> None:
    db = SessionLocal()
    try:
        ...
        print(f"done created={created} deduped={deduped} skipped={skipped} candidates={len(candidates)}")
    finally:
        db.close()


if __name__ == "__main__":
    import_broad_korean_books()
