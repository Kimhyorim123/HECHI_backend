...
def setup_user():
    ...
    login_id = f"crud_{uuid.uuid4().hex[:8]}"
    pw = "Pw123456!"
    r = client.post(
        "/auth/register",
        json={"email": email, "login_id": login_id, "password": pw, "name": "C", "nickname": "D"},
    )
    assert r.status_code in (200, 201)
    login = client.post("/auth/login", json={"login_id": login_id, "password": pw})
